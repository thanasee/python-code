...
def read_mode_kappa(filepath, n_bands):
    ...
    mode_kappa_sum = np.concatenate((
        mode_kappa.sum(axis=0)[:3, :],
        mode_kappa.sum(axis=0)[3:, :].sum(axis=0)[np.newaxis, :]
    ), axis=0)

    return mode_kappa_sum


def collect_mode_kappa(temppath, n_bands):
    
    mode_kappa_sum_temp = []
    for temppath in temppath:
        filepath = os.path.join(temppath, "BTE.kappa")
        mode_kappa_sum = read_mode_kappa(filepath, n_bands)
        mode_kappa_sum_temp.append(mode_kappa_sum)
    
    mode_kappa_sum_temp = np.array(mode_kappa_sum_temp)

    return mode_kappa_sum_temp
...
def write_cumulative_kappa_vs_mfp(filepath, mfp, cumulative_kappa):
    ...
    with open(filepath, 'w') as o:
        ...
        for m, k in zip(mfp, cumulative_kappa):
            o.write(f"{m:>10.5f}   {k[0]:>10.3f}  {k[1]:>10.3f}  {k[2]:>10.3f}"
                    f"  {k[3]:>10.3f}  {k[4]:>10.3f}  {k[5]:>10.3f}"
                    f"  {k[6]:>10.3f}  {k[7]:>10.3f}  {k[8]:>10.3f}\n")
        o.write("\n")
...
def main():
    ...
    w_isotrope_path = os.path.join(work_dir, "BTE.w_isotrope")
    frequency_w_isotrope, gamma_isotrope = read_scattering_rate(w_isotrope_path, n_qpoints, n_bands)
    write_scattering_rate_vs_frequency("ScatteringRate_IsotropeVsFrequency.dat", frequency_w_isotrope, gamma_isotrope)

    mode_kappa_sum = collect_mode_kappa(temp_dirs, n_bands)
    write_kappa_vs_temperature("Kappa_bandVsT.dat", temperatures, mode_kappa_sum)
    ...
    for temppath in temp_dirs:
        ...
        write_scattering_rate_vs_frequency(os.path.join(temppath, "ScatteringRate_3phVsFrequency.dat"), frequency_w_3ph, gamma_3ph)
        tau_3ph = compute_lifetime(gamma_3ph)
        write_lifetime_vs_frequency(os.path.join(temppath, "Lifetime_3phVsFrequency.dat"), frequency_w_3ph, tau_3ph)

        if fourphonon:
            w_4ph_path = os.path.join(temppath, "BTE.w_4ph")
            frequency_w_4ph, gamma_4ph = read_scattering_rate(w_4ph_path, n_qpoints, n_bands)
            write_scattering_rate_vs_frequency(os.path.join(temppath, "ScatteringRate_4phVsFrequency.dat"), frequency_w_4ph, gamma_4ph)
            tau_4ph = compute_lifetime(gamma_4ph)
            write_lifetime_vs_frequency(os.path.join(temppath, "Lifetime_4phVsFrequency.dat"), frequency_w_4ph, tau_4ph)

        w_path = os.path.join(temppath, "BTE.w")
        frequency_w, gamma_w = read_scattering_rate(w_path, n_qpoints, n_bands)
        write_scattering_rate_vs_frequency(os.path.join(temppath, "ScatteringRateVsFrequency.dat"), frequency_w, gamma_w)
        tau_w = compute_lifetime(gamma_w)
        write_lifetime_vs_frequency(os.path.join(temppath, "LifetimeVsFrequency.dat"), frequency_w, tau_w)
        ...
        write_scattering_rate_vs_frequency(os.path.join(temppath, "ScatteringRateFinalVsFrequency.dat"), frequency_w_final, gamma_w_final)
        tau_w_final = compute_lifetime(gamma_w_final)
        write_lifetime_vs_frequency(os.path.join(temppath, "LifetimeFinalVsFrequency.dat"), frequency_w_final, tau_w_final)
        ...
        wp3_plus_path = os.path.join(temppath, "BTE.WP3_plus") if fourphonon else os.path.join(temppath, "BTE.w_anharmonic_plus")
        ...
        wp3_minus_path = os.path.join(temppath, "BTE.WP3_minus") if fourphonon else os.path.join(temppath, "BTE.w_anharmonic_minus")
